...
- result – статус запроса. Принимает значение "true", если по данному запросу найден хотя бы один документ. В противном случае принимает значение "false", поле "relevance" отсутствует.
 - relevance — релевантность ответа. Содержит список пар "docID" и "rank". Документы отсортированы в порябке убывания rank.

 - docid — идентификатор документа, релевантного запросу. окументов исходя из порядка, в котором документы расположены в поле "files" в
 файле "config.json".

- rank — поисковый рейтинг. Отражает относительную релевантность документа.
...
Ниже приведён перечень наиболее распространённых ошибок возникающих при работе программы.
